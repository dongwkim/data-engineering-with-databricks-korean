...
# MAGIC # 노트북 이름 바꾸기
# MAGIC 
# MAGIC 노트북의 이름을 변경하는 것은 쉽습니다. 이 페이지 상단의 이름을 클릭한 다음 이름을 변경합니다. 필요한 경우 나중에 이 노트북으로 다시 쉽게 이동할 수 있도록 기존 이름 끝에 짧은 테스트 문자열을 추가합니다.
...
# MAGIC 
# MAGIC 이 페이지의 왼쪽 상단 모서리 근처에 있는 드롭다운을 클릭하여 지금 이 노트북에 클러스터를 연결합니다. 이전 실습에서 만든 클러스터를 선택합니다. 이렇게 하면 노트북의 실행 상태가 지워지고 노트북이 선택한 클러스터에 연결됩니다.
# MAGIC 
...
# MAGIC 
# MAGIC 예제 설정 노트북을 열고 이름이 **`None`**이 아니라 따옴표로 묶인 실습자의 이름(또는 다른 사람의 이름)이 되도록 수정하고 다음 두 셀이 **`AssertionError`를 발생시키지 않고 실행되도록 합니다. **.
# MAGIC 
# MAGIC <img src="https://files.training.databricks.com/images/icon_note_24.png"> **`_utility-methods`** 및 **`DBAcademyHelper`** 라는 추가 참조를 보실 수 있지만 실습을 진행하는 코스웨어이며 이 실습에서는 무시해도 됩니다. 
...
# MAGIC # 변경 사항 리뷰
# MAGIC 
# MAGIC Databricks Repo를 사용하여 이 자료를 workspace로 가져왔다고 가정하고 이 페이지의 왼쪽 상단 모서리에 있는 **`Boot-Camp-L100`** 분기 버튼을 클릭하여 Repo 대화 상자를 엽니다. 세 가지 변경 사항이 표시됩니다.
...
# MAGIC ## Wrapping Up
# MAGIC 
# MAGIC 이 실습을 완료하면 이제 노트북 조작, 새로운 셀 생성, 노트북 내에서 노트북 실행에 편안함을 느낄 수 있습니다.
