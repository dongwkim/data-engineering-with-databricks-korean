...
# MAGIC 
# MAGIC Databricks 클러스터는 다목적(Multi Purpose) 클러스터와 작업(Job) 클러스터로 구분합니다.
# MAGIC - 다목적 클러스터를 사용하여 대화형 노트북을 사용하여 공동으로 데이터를 분석합니다.
# MAGIC - 작업 클러스터를 사용하여 빠르고 강력한 자동화 작업을 실행합니다.
# MAGIC 
# MAGIC 이 데모에서는 Databricks Data Science & Engineering Workspace를 사용하여 다목적(Multi Purpose) Databricks 클러스터를 만들고 관리하는 방법을 다룹니다.
# MAGIC 
# MAGIC ## 학습 목표
...
# MAGIC 
# MAGIC 이 섹션의 지침은 클러스터 생성 권한이 **있고** 이 과정의 단원을 실행하기 위해 새 클러스터를 배포해야 한다고 가정합니다.
# MAGIC 
# MAGIC **참고**: 강사 또는 플랫폼 관리자에게 문의하여 새 클러스터를 만들어야 하는지 또는 이미 배포된 클러스터에 연결해야 하는지 확인하십시오. 클러스터 정책은 클러스터 구성 옵션에 영향을 줄 수 있습니다.
# MAGIC 
# MAGIC 단계:
...
# MAGIC 1. **Cluster name**은 본인의 이름을 쉽게 찾을 수 있고 문제가 있을 경우 강사가 쉽게 식별할 수 있는 이름을 사용합니다.
# MAGIC 1. **Cluster Mode**를 **Single Node**로 설정합니다(이 과정을 실행하려면 이 모드가 필요함).
# MAGIC 1. 이 과정에 권장되는 **Databricks runtime version** 사용
# MAGIC 1. **Autopilot Options** 아래의 기본 설정 상자를 선택된 상태로 둡니다.
# MAGIC 1. 파란색 **Create Cluster** 버튼을 클릭합니다.
# MAGIC 
...
# MAGIC 
# MAGIC 현재 구성을 검토하려면 클러스터를 선택하세요.
# MAGIC 
# MAGIC **Edit** 버튼을 클릭합니다. 대부분의 설정은 수정할 수 있습니다(충분한 권한이 있는 경우). 대부분의 설정을 변경하려면 실행 중인 클러스터를 다시 시작해야 합니다.
# MAGIC 
# MAGIC **참고**: 다음 레슨에서는 지금 만들어진 클러스터를 사용할 것입니다. 클러스터를 다시 시작, 종료 또는 삭제하면 새 리소스가 배포될 때까지 기달려야 할 수 있습니다.
...
# MAGIC 
# MAGIC **Restart**, **Terminate** 및 **Delete**의 효과는 다르지만 모두 클러스터 종료 이벤트로 시작합니다. (이 설정을 사용한다고 가정하면 비활성으로 인해 클러스터도 자동으로 종료됩니다.)
# MAGIC 
...
# MAGIC 
# MAGIC 즉, 컴퓨팅 환경과 이전에 연결된 모든 리소스가 완전히 제거됩니다. 즉, **유지해야 하는 모든 결과는 영구적인 위치에 저장해야 합니다**. 여러분의 코드는 손실되지 않으며 저장한 데이터 파일도 손실되지 않습니다.
# MAGIC 
